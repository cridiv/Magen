...
export class BriefsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  emitPipelineError(detail: { tokenAddress: string; reason: string }): void {
    ...
    this.logger.warn(
      `Emitted pipeline:error — ${detail.reason} for ${detail.tokenAddress}`,
    );
  }

  emitAgentLog(log: AgentLogEvent): void {
    this.server.emit('backend:log', log);
    this.server.emit('agent:log', log);
    this.logger.debug(
      `Emitted agent:log ${log.eventType}${log.tokenAddress ? ` for ${log.tokenAddress}` : ''}`,
    );
  }
}
